...
// ✅ FIX: Added WidgetsBindingObserver to detect app lifecycle changes directly in the chat
class _ChatScreenState extends ConsumerState<ChatScreen> with WidgetsBindingObserver {
  ...
  Timer? _presenceTimer; // ✅ FIX: Added aggressive polling timer
  ...
  void initState() {
    super.initState();
    WidgetsBinding.instance.addObserver(this); // ✅ Bind lifecycle observer
  ...
  // ✅ FIX: Force a presence check if the user resumes the app straight into this chat
  @override
  ...
  // ✅ FIX: Centralized presence request function
  void _requestPresence() {
    if (widget.isGroup || !mounted) return;
    try {
      SocketService().checkUserStatus(widget.receiverId);
      // Aggressively emit directly in case SocketService is lagging
      SocketService().socket?.emit('check_user_status', {'userId': widget.receiverId});
    } catch (e) {
      debugPrint("Presence request error: $e");
    }
  }

  // ✅ FIX: Shared handler for incoming socket presence events
  ...
  void _setupLivePresence() {
    ...
    _statusSubscription = SocketService().userStatusStream.listen(_handleDirectPresenceUpdate);
  ...
  void dispose() {
    ...
    _presenceTimer?.cancel(); // ✅ Clean up polling timer
