...
class _ProfileScreenState extends ConsumerState<ProfileScreen> with WidgetsBindingObserver {
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.resumed) {
      _checkNotificationStatus();
      // ✅ FIX: Force the profile to ask the server for its presence the moment the app resumes
      ref.read(profileProvider.notifier).refreshPresence(); 
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    // ✅ ANTI-FLASH SKELETON: Only show skeleton if we are loading AND we have zero cached data.
    if (profileState.isLoading && (userProfile == null || userProfile.isEmpty)) {
      return Scaffold(backgroundColor: scaffoldBg, body: const ProfileSkeleton());
    }
